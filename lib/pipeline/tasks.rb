...
class Pipeline::Tasks
  ...
  def self.run_tasks(target, stage, tracker)
    ...
    self.tasks_to_run(tracker).each do |c|
      ...
      unless skip_tasks.include?(task_name.downcase) ||
             (run_tasks && (not run_tasks.include? task_name.downcase))

        task = c.new(trigger, tracker)
        begin
          if task.supported? && task.stage == stage  
            if task.labels.intersect?(tracker.options[:labels]) || # Only run tasks with labels
               ( run_tasks && run_tasks.include?(task_name.downcase) ) # or that are explicitly requested.
  ...
  def self.tasks_to_run(tracker)
    if tracker.options[:run_all_tasks] || tracker.options[:run_tasks]
      @tasks + @optional_tasks
    else
      @tasks
    end
