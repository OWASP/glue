...
describe Glue::Snyk do
  ...
  def get_target_path(target, subtarget = nil)
    if subtarget.nil?
      File.join(TARGETS_PATH, target)
    else
      File.join(TARGETS_PATH, target, subtarget)
    end
  end
  ...
  describe "#initialize" do
    let(:task) { @task }
    before(:all) { @task = get_snyk }

    it "sets the correct 'name'" do
      expect(task.name).to eq('Snyk')
    end
  ...
  describe "#supported?" do
    ...
    subject(:task) { get_snyk }

    context "when 'find_executable0' cannot find 'snyk'" do
      before do
        allow(Glue).to receive(:notify) # stub Glue.notify to prevent printing to screen
        allow_any_instance_of(Object).to receive(:find_executable0).with('snyk').and_return(nil)
      end
    ...
    context "when 'find_executable0' can find 'snyk'" do
      before do
        allow_any_instance_of(Object).to receive(:find_executable0).with('snyk').and_return('/usr/bin/snyk')
      end
